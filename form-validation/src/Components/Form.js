...
const Form = () => {
  const [formData, setFormData] = useState({
    firstName: "", lastName: "", email: "", phoneNumber: "", password: "", confirmPassword: "", age: "", gender: "", interests: [], birthDate: "",
  });
  ...
  const isValidPassword = (password) => {
    ...
    return (
      password.length >= 8 && symbolRegex.test(password) && numberRegex.test(password) && upperCaseRegex.test(password) && lowerCaseRegex.test(password)
    );
  };
  ...
  const handleCheckboxChange = (e) => {
    ...
    setFormData({
      ...formData, interests: updatedInterests,
    });
  };
  ...
  return (
    <form className="form" onSubmit={handleSubmit}>
      ...
      <div>
        ...
        <div className="position-rel-abs">
        <input type={isVisibilityPassword ? "text" : "password"} name="password" value={formData.password} placeholder="Enter your password" onChange={handleChange}/>{isVisibilityPassword ? <FaEyeSlash onClick={()=>setIsVisibilityPassword(false)} /> : <FaEye onClick={()=>setIsVisibilityPassword(true)} />}
        </div>
      ...
      <div>
        ...
        <div className="position-rel-abs">
        <input type={isVisibilityConfirmPassword ? "text" : "password"} name="confirmPassword" value={formData.confirmPassword} placeholder="Confirm your password" onChange={handleChange}/>{isVisibilityConfirmPassword ? <FaEyeSlash onClick={()=>setIsVisibilityConfirmPassword(false)} /> : <FaEye onClick={()=>setIsVisibilityConfirmPassword(true)} />}
        </div>
